...
df = pd.read_csv('jobs_Data/global_ai_ml_data_salaries.csv')
...
with tab2:
    with st.container():
        ...
        with col1: 
            ...
            fig1 = px.pie(df1_aux, values='percent', names='remote_ratio', 
                          color='remote_ratio', 
            ...
            st.markdown('##### Proporção de Home Office para Engenheiro de Dados por tamanho de empresa e nível de experiência')
            ...
            fig5, ax = plt.subplots(figsize=(5, 6))
        ...
        with col2:
            ...
            def calculate_remote_percent(year):
                lines_remote = (df1['remote_ratio'] == 'Home Office') & (df1['work_year'] == year)
                count_remote = df1[lines_remote]['remote_ratio'].count()
                line_year = (df1['work_year'] == year)
                count_total = df1[line_year]['remote_ratio'].count()
                return pd.DataFrame({'Remote': [count_remote],
                                     'All Modalities': [count_total],
                                     'Year': [year],
                                     'Remote_percent': [round(count_remote / count_total * 100, 2)]})

            df_concat = pd.concat([calculate_remote_percent(year) for year in range(2020, 2025)], axis=0)
            fig3 = px.bar(df_concat, x='Year', y='Remote_percent', text='Remote_percent')
            fig3.update_traces(textposition='outside')
            st.plotly_chart(fig3, use_container_width=True)
            
            st.markdown('##### Proporção de Home Office para Cientista de Dados por tamanho de empresa e nível de experiência')
            ...
            fig4, ax = plt.subplots(figsize=(5, 6))
            df_relative.plot(kind='bar', stacked=True, colormap='viridis', ax=ax)
            ax.set_xlabel('Tamanho da empresa')
            ax.set_ylabel('Percentual de contratações')
            ax.legend(title='Nível', bbox_to_anchor=(1.05, 1), loc='upper left')
            ...
            fig6, ax = plt.subplots(figsize=(5, 6))
            df_relative.plot(kind='bar', stacked=True, colormap='viridis', ax=ax)
            ax.set_xlabel('Tamanho da empresa')
            ax.set_ylabel('Percentual de contratações')
            ax.legend(title='Nível', bbox_to_anchor=(1.05, 1), loc='upper left')
